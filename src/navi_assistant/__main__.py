...
def setup_api_key(config: dict[str, dict[str, str]]) -> None:
    existing_key = config.get("openai", {}).get("api_key", None)
...
def setup_assistant(config: dict[str, dict[str, str]], client: OpenAI) -> None:
...
def setup_new_assistant(config: dict[str, dict[str, str]], client: OpenAI) -> None:
...
def setup_existing_assistant(config: dict[str, dict[str, str]], client: OpenAI) -> None:
...
def ask(query: str):
    ...
    display_messages(client, config, thread_id)
...
def display_messages(client: OpenAI, config: dict[str, dict[str, str]], thread_id: str):
    if config["openai"]["last_message_id"]:
        messages = client.beta.threads.messages.list(
            thread_id, after=config["openai"]["last_message_id"], order="asc"
        )
    else:
